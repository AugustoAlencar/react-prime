...
function Home() {
  const [nowMovies, setNowMovies]: Array<any> = useState([]);
  const [popularMovies, setPopularMovies]: Array<any> = useState([]);
  const [topMovies, setTopMovies]: Array<any> = useState([]);
  const [bannerMovie, setBannerMovie]: any = useState({});
  ...
  useEffect(() => {
    ...
    async function getMovies() {
      ...
      const [nowData, popularData, topData]: any = await Promise.all([
    ...
    <Container>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <BannerButton
          activeOpacity={0.9}
          onPress={() => {
            navigateDetailsPage(bannerMovie);
          }}
        >
          <Banner
            resizeMethod="resize"
